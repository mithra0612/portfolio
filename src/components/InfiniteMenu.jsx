...
class DiscGeometry extends Geometry {
  constructor(steps = 4, radius = 1) {
    ...
    const size = radius * 2; // Make it a square
    const cornerRadius = radius * 0.2; // Rounded corner radius
    ...
    const addCornerArc = (cx, cy, startAngle, endAngle, segments) => {
      for (let i = 0; i <= segments; i++) {
        const t = i / segments;
        const angle = startAngle + (endAngle - startAngle) * t;
        const x = cx + cornerRadius * Math.cos(angle);
        const y = cy + cornerRadius * Math.sin(angle);
        vertices.push({ x, y });
      }
    };
    ...
    // Top-right corner
    addCornerArc(inner, inner, 0, Math.PI / 2, segmentsPerCorner);
    
    // Top-left corner
    addCornerArc(-inner, inner, Math.PI / 2, Math.PI, segmentsPerCorner);
    
    // Bottom-left corner
    addCornerArc(-inner, -inner, Math.PI, Math.PI * 1.5, segmentsPerCorner);
    
    // Bottom-right corner
    addCornerArc(inner, -inner, Math.PI * 1.5, Math.PI * 2, segmentsPerCorner);
    
    // Add all vertices
    vertices.forEach(v => {
      this.addVertex(v.x, v.y, 0);
      // UV mapping
      this.lastVertex.uv[0] = (v.x / size) + 0.5;
      this.lastVertex.uv[1] = (v.y / size) + 0.5;
    });
    
    // Create faces from center to perimeter
...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    ...
    this.discGeo = new DiscGeometry(30, 1);
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Note: WebGL resources cleanup not implemented in your original code.
    };
  }, [items, scale]);
