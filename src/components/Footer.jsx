...
const Footer = () => {
    ...
    return (
      ...
      >
        {text.split('').map((char, index) => (
          <span
            key={index}
            className={`inline-block transition-all duration-300 ease-out font-bold group-hover:text-tan ${
  ...
  const NavLink = ({ href, children, onClick }) => (
    <a 
      href={href}
      onClick={onClick}
      className="text-sm md:text-base font-medium leading-tight tracking-tight text-soft-linen no-underline transition-all duration-500 ease-out"
    >
      <BounceText text={children} />
    </a>
  );
  return (
    <footer className="relative bg-black text-soft-linen/80 py-6 md:py-8">
            ...
            <div className="order-1 md:order-2 text-center md:text-right">
              <div className="flex flex-col md:items-end space-y-3">
                <div className="flex justify-center md:justify-end space-x-4">
                  <a href="#" className="text-soft-linen/70 hover:text-tan transition-colors duration-300">
                ...
                <p className="text-sm text-soft-linen/70">
                  &copy; {new Date().getFullYear()} • Crafted with <span className="text-tan animate-pulse">❤</span>
                </p>
              </div>
            </div>
          </div>
